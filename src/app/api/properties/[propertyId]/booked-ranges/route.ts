...
export async function GET(
  ...
  try {
    await connectDB();
    console.log(`[API /properties/${propertyId}/booked-ranges GET] Attempting to fetch booked ranges for property ID: ${propertyId}`);
    ...
    console.log(`[API /properties/${propertyId}/booked-ranges GET] Transformed booked ranges:`, JSON.stringify(bookedRanges, null, 2).substring(0, 500) + (JSON.stringify(bookedRanges, null, 2).length > 500 ? '...' : ''));


    return NextResponse.json(bookedRanges, { status: 200 });

  } catch (error: any) {
    console.error(`[API /properties/${propertyId}/booked-ranges GET] Critical error fetching booked ranges:`, error.message, error.stack);
