...
export async function GET(
  request: NextRequest,
  { params }: { params: { id: string } } // Changed propertyId to id
) {
  const { id } = params; // Changed propertyId to id

  if (!id || !mongoose.Types.ObjectId.isValid(id)) { // Changed propertyId to id
    return NextResponse.json({ message: 'Invalid property ID format.' }, { status: 400 });
  }

  try {
    await connectDB();
    console.log(`[API /properties/${id}/booked-ranges GET] Attempting to fetch booked ranges for property ID: ${id}`); // Changed propertyId to id
    ...
    const bookedRangesDocs = await BookedDateRangeModel.find({
      propertyId: new mongoose.Types.ObjectId(id), // Changed propertyId to id
      status: { $in: relevantStatuses }
    }).lean();

    console.log(`[API /properties/${id}/booked-ranges GET] Found ${bookedRangesDocs.length} relevant booked range documents.`); // Changed propertyId to id
    ...
    console.log(`[API /properties/${id}/booked-ranges GET] Transformed booked ranges:`, JSON.stringify(bookedRanges, null, 2).substring(0, 500) + (JSON.stringify(bookedRanges, null, 2).length > 500 ? '...' : '')); // Changed propertyId to id


    return NextResponse.json(bookedRanges, { status: 200 });

  } catch (error: any) {
    console.error(`[API /properties/${id}/booked-ranges GET] Critical error fetching booked ranges:`, error.message, error.stack); // Changed propertyId to id
